...
# Genuine iPad assets: locale-independent iPad raws (host app + keyboard with the
# 360pt trailing side-panel layout) live in marketing/raw/ipad/, generated by
# marketing/generate_ipad_raws.py. The iPad device frame is generated on demand.
IPAD_RAW = ROOT / "marketing" / "raw" / "ipad"
IPAD_RAW_SLUGS = ["01-hero", "02-checkout", "03-taxtip", "04-clipboard", "05-packs", "06-themes"]
...
def gradient(top: tuple[int, int, int], bottom: tuple[int, int, int], w: int, h: int) -> Image.Image:
    try:
        ...
        rows = np.array(top, dtype=np.float32)[None, :] * (1 - t) + np.array(bottom, dtype=np.float32)[None, :] * t
        arr = np.ascontiguousarray(np.broadcast_to(rows[:, None, :], (h, w, 3))).round().astype(np.uint8)
...
def _build_ipad_frame() -> None:
    """Generate a clean portrait iPad Pro device frame (no notch / no home button)."""
    sw, sh, b = _IPAD_SCREEN_W, _IPAD_SCREEN_H, _IPAD_BEZEL
    W, H = sw + 2 * b, sh + 2 * b
    frame = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    d = ImageDraw.Draw(frame)
    # graphite aluminium body
    d.rounded_rectangle((0, 0, W - 1, H - 1), radius=_IPAD_FRAME_RADIUS, fill=(36, 37, 40, 255))
    # outer rim highlight
    d.rounded_rectangle((3, 3, W - 4, H - 4), radius=_IPAD_FRAME_RADIUS - 3,
                        outline=(78, 80, 86, 255), width=3)
    # black screen well (slightly larger than the screen so a thin bezel shows)
    d.rounded_rectangle((b - 6, b - 6, b + sw + 6, b + sh + 6),
                        radius=_IPAD_SCREEN_RADIUS + 4, fill=(0, 0, 0, 255))
    # front camera dot, top-centre bezel
    cx = W // 2
    d.ellipse((cx - 7, b // 2 - 7, cx + 7, b // 2 + 7), fill=(12, 12, 14, 255))
    IPAD_MOCKUP.parent.mkdir(parents=True, exist_ok=True)
...
def make_ipad(source: Path, width: int) -> Image.Image:
    ...
    fw, fh = _IPAD_SCREEN_W + 2 * _IPAD_BEZEL, _IPAD_SCREEN_H + 2 * _IPAD_BEZEL
    sc_l = round((_IPAD_BEZEL / fw) * device.width)
    sc_t = round((_IPAD_BEZEL / fh) * device.height)
    sc_w = round((_IPAD_SCREEN_W / fw) * device.width)
    sc_h = round((_IPAD_SCREEN_H / fh) * device.height)
    screen = shot.resize((sc_w, sc_h), Image.Resampling.LANCZOS)
    mask = Image.new("L", (sc_w, sc_h), 0)
    ImageDraw.Draw(mask).rounded_rectangle((0, 0, sc_w, sc_h), radius=round(sc_w * 0.027), fill=255)
    device.paste(screen, (sc_l, sc_t), mask)
    return device
...
def render_slide_ipad(slide: Slide, locale: str = "en", slide_idx: int = 0) -> Image.Image:
    ...
    avail_top = sub_bottom + round(48 * sy)
    avail_h = (footer_y1 - round(56 * sy)) - avail_top
...
def generate_locale(locale: str, ipad_only: bool = False) -> tuple[int, int]:
    """Generate all iPhone and iPad screenshots for a single locale.

    When ``ipad_only`` is set, the iPhone slides (unchanged) are skipped so that
    an iPad-frame refresh can regenerate only the 12 iPad files per locale.
...
def main() -> None:
    ...
    ipad_only = "--ipad-only" in sys.argv

    total_iphone = 0
    total_ipad = 0
    for locale in locales:
        iphone, ipad = generate_locale(locale, ipad_only=ipad_only)
        total_iphone += iphone
        total_ipad += ipad
