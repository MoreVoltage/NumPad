...
struct StoreKit {
    enum Product {
        ...
        static let all: [Product] = [mathPack]
        
        var identifier: String {
            switch self {
            case .mathPack:
                return "com.morevoltage.numpad.MathPack"
            }
        }
    }
    
    static func getInfo(products: [Product], completion: @escaping ([SKProduct]?) -> Void) {
    ...
    static func purchase(product: Product, completion: () -> Void) {
    ...
    static func restorePurchases(completion: () -> Void) {
