...
struct QwertyPersonalDictionary: Codable, Equatable {
    ...
    /// Recordings between halvings — words age out unless re-typed.
    static let decayInterval = 200
    /// Count at which a word is "known": protected from autocorrect.
    static let protectionThreshold = 3

    // MARK: - Recording

    /// Counts one acceptance of `word` (lowercased). Junk never counts: only letters plus
    /// the in-word characters autocorrect itself accepts (' \u{2019} -), length 2–24, and at
    /// least one letter.
    mutating func recordAcceptance(of word: String) {
        guard let normalized = Self.normalize(word) else { return }
    ...
    /// The learned count for `word` (case-insensitive); 0 when absent.
    func boost(for word: String) -> Int {
        counts[word.lowercased()] ?? 0
    }
    ...
    /// Lowercased; letters plus in-word ' \u{2019} - only; length 2–24; at least one letter
    /// (pure punctuation is junk). Mirrors QwertyAutocorrect's word-internal character set.
    private static func normalize(_ word: String) -> String? {
        ...
        let wordInternal: Set<Character> = ["'", "\u{2019}", "-"]
